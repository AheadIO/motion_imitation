...
"""Pybullet simulation of a Stella robot."""
import math
import os
import re
import numpy as np
...
from motion_imitation.robots import stella_pose_utils
from motion_imitation.robots import stella_constants
from motion_imitation.robots import stella_motor
...
# Bases on the readings from Stella's default pose.
INIT_MOTOR_ANGLES = np.array([
    stella_pose_utils.STELLA_DEFAULT_ABDUCTION_ANGLE,
    stella_pose_utils.STELLA_DEFAULT_HIP_ANGLE,
    stella_pose_utils.STELLA_DEFAULT_KNEE_ANGLE
] * NUM_LEGS)
...
URDF_FILENAME = "stella/stella_toes_limits.urdf"
...
class Stella(minitaur.Minitaur):
  """A simulation for the Stella robot."""
  ...
  def __init__(
    ...
    super(Stella, self).__init__(
        ...
        motor_model_class=stella_motor.StellaMotorModel,
        sensors=sensors,
        motor_kp=motor_kp,
        motor_kd=motor_kd,
  ...
  def _LoadRobotURDF(self):
    stella_urdf_path = self.GetURDFFile()
    if self._self_collision_enabled:
      ...
      self.quadruped = self._pybullet_client.loadURDF(
          stella_urdf_path, self._GetDefaultInitPosition(),
          self._GetDefaultInitOrientation())
  ...
  def ComputeJacobian(self, leg_id):
    ...
    # Because of the default rotation in the Stella URDF, we need to reorder
    # the rows in the Jacobian matrix.
    return super(Stella, self).ComputeJacobian(leg_id)[(2, 0, 1), :]
  ...
  def _GetDefaultInitOrientation(self):
    # The Stella URDF assumes the initial pose of heading towards z axis,
    # and belly towards y axis. The following transformation is to transform
    # the Stella initial orientation to our commonly used orientation: heading
  ...
  def ApplyAction(self, motor_commands, motor_control_mode):
    ...
    super(Stella, self).ApplyAction(motor_commands, motor_control_mode)
