...
STELLA_DEFAULT_ABDUCTION_ANGLE = 0
STELLA_DEFAULT_HIP_ANGLE = 0.67
STELLA_DEFAULT_KNEE_ANGLE = -1.25
NUM_LEGS = 4
NUM_MOTORS = 12
# Bases on the readings from Stella's default pose.
INIT_MOTOR_ANGLES = np.array([
    STELLA_DEFAULT_ABDUCTION_ANGLE,
    STELLA_DEFAULT_HIP_ANGLE,
    STELLA_DEFAULT_KNEE_ANGLE
] * NUM_LEGS)
MOTOR_NAMES = [
...
class StellaMotorModel(object):
  """A simple motor model for Stella.
...
class SimpleRobot(object):
  def __init__(self, pybullet_client, robot_uid, simulation_time_step):
    ...
    self._motor_model = StellaMotorModel(kp=self._kp, kd=self._kd, motor_control_mode=MOTOR_CONTROL_HYBRID)
  ...
  def _EndEffectorIK(self, leg_id, position, position_in_world_frame):
    ...
    # Joint offset is necessary for Stella.
