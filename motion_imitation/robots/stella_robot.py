...
"""Real robot interface of Stella robot."""
...
from motion_imitation.robots import stella
from motion_imitation.robots import stella_robot_velocity_estimator
...
URDF_FILENAME = "stella/stella.urdf"

_BODY_B_FIELD_NUMBER = 2
_LINK_A_FIELD_NUMBER = 3


class StellaRobot(stella.Stella):
  """Interface for real Stella robot."""
  ...
  def __init__(self, pybullet_client, time_step=0.002, **kwargs):
    ...
    self._velocity_estimator = stella_robot_velocity_estimator.VelocityEstimator(
        self)
    ...
    kwargs['on_rack'] = True
    super(StellaRobot, self).__init__(pybullet_client,
                                  time_step=time_step,
                                  **kwargs)
  ...
  def ReceiveObservation(self):
    ...
    Synchronous ReceiveObservation is not supported in Stella,
  ...
  def ApplyAction(self, motor_commands, motor_control_mode=None):
    ...
    if motor_control_mode == robot_config.MotorControlMode.POSITION:
      ...
      raise ValueError('Unknown motor control mode for Stella robot: {}.'.format(
          motor_control_mode))
  ...
  def Reset(self, reload_urdf=True, default_motor_angles=None, reset_time=3.0):
    ...
    super(StellaRobot, self).Reset(reload_urdf=reload_urdf,
    ...
    if not default_motor_angles:
      default_motor_angles = stella.INIT_MOTOR_ANGLES
